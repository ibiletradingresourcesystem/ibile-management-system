 ...
 * - Pack Qty / Parent cols → mother (pack) and child relationships. Children never hold stock;
 *                            their qty comes from the parent, so a child's Qty cell is ignored.
 */
import { formatBarcodes, splitStoredBarcodes } from "@/lib/barcodes";
 ...
 * @param {object}   args.options           { canSeedQty, updateExistingQty }
 */
export function buildImportPlan({ rows = [], existingProducts = [], options = {} }) {
  const { canSeedQty = false, updateExistingQty = false } = options;

  const byName = new Map();
  const byBarcode = new Map();
  const byId = new Map();
  const childUnitsByParent = new Map(); // parentId -> [units of each active child]
  ...
  for (const product of existingProducts) {
    ...
    if (isDerivedChild(product) && !product.isArchived) {
      ...
      childUnitsByParent.set(parentId, [...(childUnitsByParent.get(parentId) || []), getUnitsPerChild(product)]);
    }
  }
  ...
  for (const entry of entries) {
    ...
    if (entry.product && isDerivedChild(entry.product)) {
      fail(entry, "This product is linked as a child in the system — unlink it before making it a pack");
      continue;
    }
    const existingChildUnits = entry.product ? childUnitsByParent.get(String(entry.product._id)) || [] : [];
    const largestChild = Math.max(0, ...existingChildUnits);
  ...
  for (const entry of entries) {
    ...
    if (!parentEntry) {
      // The parent may be referenced by its name in the system while its row in the file differs
      const existingParent = findExisting(existingIndex, ref, [ref]).product;
      if (existingParent) parentEntry = entryByProductId.get(String(existingParent._id)) || null;
    }
    ...
    if (parentEntry) {
      ...
      const match = findExisting(existingIndex, ref, [ref]);
      if (match.error || !match.product) {
        fail(entry, match.error ? `Parent "${ref}": ${match.error}` : `Parent "${ref}" not found in the file or in the system`);
        continue;
      }
      const parentProduct = match.product;
    ...
    if (entry.product && (childUnitsByParent.get(String(entry.product._id)) || []).length > 0) {
  ...
  for (const entry of entries) {
    ...
    const takesStockFromParent = isChild || (entry.product && isDerivedChild(entry.product));

    if (row.quantity !== null) {
      if (takesStockFromParent) entry.warnings.push("Child stock comes from its parent pack — Qty ignored");
      else if (!canSeedQty) entry.warnings.push("Only admins can seed stock — Qty ignored");
    }
