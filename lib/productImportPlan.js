...
export function buildImportPlan({ rows = [], existingProducts = [], options = {} }) {
  ...
  for (const entry of entries) {
     ...
     */
    const declaresFamily = Boolean(entry.row.parentRef) || (entry.row.packQty !== null && entry.row.packQty > 1);
    ...
    if (match.product) {
      ...
      // Family rows never arrive here on a barcode match, so a collision at this point means two
      // rows genuinely point at one product, which is a real mistake in the file.
