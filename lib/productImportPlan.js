...
export function buildImportPlan({ rows = [], existingProducts = [], options = {} }) {
  ...
  for (const entry of entries) {
     ...
     *
     * Structural instructions in the file are deliberate, not price drift, so a row carrying a
     * Parent or Pack Qty cell is always honoured.
     */
    ...
    if (skipUnchangedCost && costUnchanged && !structuralRow) {
