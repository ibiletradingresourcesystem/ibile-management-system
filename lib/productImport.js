...
/**
 * How many units one pack holds, read out of a product's own name or description.
 *
 * Catalogues name the pack size rather than putting it in a column: "(48 units)",
 * "(PACK OF 24)", "(Pack of 12)", "(40 n pack)", "(PACK x24)", "(x200)". When the Pack Qty cell
 * is blank this is the only statement of the size in the file, so it is read as a fallback and
 * always reported, never applied silently.
 *
 * @param {...string} texts name, description, anything else that might carry the size
 * @returns {{ size: number, from: string } | null}
 */
export function parsePackSizeFromText(...texts) {
  for (const raw of texts) {
    const text = String(raw ?? "").toLowerCase();
    if (!text) continue;

    // "(x200)" — an x-number in brackets is unambiguous on its own
    const bracketed = text.match(/\(\s*x\s*(\d{1,4})\s*\)/);
    if (bracketed) {
      const size = Number(bracketed[1]);
      if (size > 1) return { size, from: bracketed[0].trim() };
    }

    // Everything else has to sit next to a word that means a pack, so that a
    // "440ml" or a "3ply" is never mistaken for a quantity.
    if (!/pack|unit|roll|carton|ctn|crate|dozen|tray/.test(text)) continue;

    const patterns = [
      /(?:pack|packs|carton|ctn|crate|tray|roll)\s*(?:of|x|n)?\s*(\d{1,4})\b/, // pack of 24, packs x24, roll of 10
      /\b(\d{1,4})\s*(?:n|in)?\s*(?:pack|packs|carton|ctn|crate|tray)\b/, // 40 n pack, 12 pack
      /\b(\d{1,4})\s*units?\b/, // 48 units
      /\bx\s*(\d{1,4})\b/, // pack x24
    ];
    ...
    for (const pattern of patterns) {
      ...
      // A "1 unit" names the single, not a pack, so it is not a size.
      if (size > 1 && size <= 10000) return { size, from: match[0].trim() };
    }
  }
  return null;
}
...
export function classifySkipReason(message) {
  ...
  if (text.includes("isn't a pack") || text.includes("is not a pack yet")) {
    return bucket(
      "parent-not-pack",
      "The parent is not a pack",
      "Give the mother row a Pack Qty saying how many units one pack holds. Use 1 for a set sold whole or as a part (a dispenser with its bottle, and the dispenser alone), where each sale takes 1 off the same stock."
    );
  }
  ...
  if (text.startsWith("pack qty ")) {
    return bucket(
      ...
      "Pack Qty must be at least as large as the biggest existing child. A Pack Qty of 1 makes a pack only on a row another row names as its Parent."
    );
  }
