...
import { calculateMarginPercent, calculateProfitMarginPercent, normalizeTaxRate } from "@/lib/pricing";
...
- currentMargin is the mark-up added on cost, before VAT; currentProfitMargin is the share of the
  sale price kept as profit. Selling prices include vatRate% VAT, which is not profit.
- Quote margins back as currentMargin (mark-up on cost), so they match the product screen
...
export async function generatePricingRecommendation(product, options = {}) {
  ...
  return executeRecommendationPipeline({
    ...
    formatResult: (aiData) => ({
      recommendation: `${aiData.strategy === "increase" ? "Increase" : aiData.strategy === "decrease" ? "Decrease" : "Maintain"} price to â‚¦${Number(aiData.recommendedPrice || 0).toLocaleString()} (${aiData.recommendedMargin || 0}% margin)`,
...
function buildPricingMetrics(product) {
  ...
  const margin = calculateMarginPercent(costPrice, sellingPrice, taxRate);
  const profitMargin = calculateProfitMarginPercent(costPrice, sellingPrice, taxRate);
  ...
  return {
    ...
    currentProfitMargin: Math.round(profitMargin * 10) / 10,
    vatRate: taxRate,
