...
- currentMargin is the share of the sale price kept as profit, worked out after taking VAT off:
  (price - VAT - cost) / (price - VAT). currentMarkup is that same profit read against cost.
- Selling prices include vatRate% VAT, which is collected for the taxman and is not profit
- Quote margins back on the currentMargin basis, so they match the product screen
...
function buildPricingMetrics(product) {
  ...
  const markup = calculateMarkupPercent(costPrice, sellingPrice, taxRate);
