 ...
 * Gracefully falls back to no-AI when no provider is reachable.
 */
...
Return JSON:
{
  ...
  "opportunities": [
    {"title": "opportunity", "description": "explanation", "potentialBenefit": "â‚¦ or % estimate", "effort": "low/medium/high"}
  ],
  "recommendations": {
    "immediate": [{"action": "what to do", "reason": "why", "expectedImpact": "â‚¦ or % benefit", "confidence": 0-100}],
    "shortTerm": [{"action": "what to do", "reason": "why", "expectedImpact": "â‚¦ or % benefit", "confidence": 0-100}],
    "longTerm": [{"action": "what to do", "reason": "why", "expectedImpact": "â‚¦ or % benefit", "confidence": 0-100}]
  },
  "healthScore": 0-100,
...
export async function generateMonthlyReportInsight(monthlyMetrics) {
  ...
  try {
    ...
    // Generate a new insight through whichever provider is configured
    const prompt = MONTHLY_REPORT_PROMPT + JSON.stringify(monthlyMetrics, null, 2);
    const aiResult = await generateAIText(prompt, { json: true });

    if (!aiResult.success) {
      console.warn("[Monthly AI]", aiResult.error);
      return null;
    }
...
export function buildMonthlyReportAIHtml(insight) {
  ...
  const outlookEmoji = insight.growthOutlook === "positive" ? "ðŸ“ˆ" : insight.growthOutlook === "negative" ? "ðŸ“‰" : "âž¡ï¸";
  ...
  let html = `
    ...
    <div style="background: linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%); color: white; padding: 30px; border-radius: 10px; margin-bottom: 20px;">
      <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 20px;">
        <h2 style="margin: 0; font-size: 20px;">ðŸ¤– AI Executive Briefing</h2>
      ...
      ${insight.stale ? '<p style="margin-top: 10px; font-size: 11px; opacity: 0.6;">âš ï¸ Using cached analysis from previous period</p>' : ""}
    </div>`;
  ...
  if (insight.risks?.length > 0) {
    html += `
    <div style="background: white; padding: 25px; border-radius: 10px; margin-bottom: 20px; border: 1px solid #e5e7eb;">
      <h3 style="margin: 0 0 15px 0; font-size: 16px; color: #1f2937;">âš ï¸ Business Risks</h3>
      ${insight.risks.slice(0, 6).map(r => `
        <div style="padding: 10px 14px; margin-bottom: 8px; background: #fef2f2; border-radius: 6px; border-left: 3px solid ${r.severity === "high" ? "#dc2626" : "#f59e0b"};">
          ...
          ${r.mitigation ? `<p style="margin: 4px 0 0 0; font-size: 11px; color: #059669;">ðŸ’¡ ${r.mitigation}</p>` : ""}
        </div>
      `).join("")}
    </div>`;
  }
  ...
  if (insight.opportunities?.length > 0) {
    html += `
    <div style="background: white; padding: 25px; border-radius: 10px; margin-bottom: 20px; border: 1px solid #e5e7eb;">
      <h3 style="margin: 0 0 15px 0; font-size: 16px; color: #1f2937;">ðŸ’¡ Opportunities</h3>
  ...
  if (allRecs.length > 0) {
    html += `
    <div style="background: white; padding: 25px; border-radius: 10px; margin-bottom: 20px; border: 1px solid #e5e7eb;">
      <h3 style="margin: 0 0 15px 0; font-size: 16px; color: #1f2937;">ðŸ“‹ Manager Recommendations</h3>`;
    ...
    if (recs.immediate?.length > 0) {
      ...
      html += recs.immediate.slice(0, 3).map(r => `
        <div style="padding: 8px 12px; margin-bottom: 6px; background: #fef2f2; border-radius: 4px;">
          ...
          <p style="margin: 2px 0 0 0; font-size: 11px; color: #6b7280;">${r.reason || ""} ${r.expectedImpact ? `â€¢ Impact: ${r.expectedImpact}` : ""}</p>
        </div>
      `).join("");
    }
